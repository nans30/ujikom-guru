...
class AttendanceController extends Controller
{
    ...
    */
    public function scan(Request $request)
    {
        ...
        try {
            ...
            if (!$attendance) {
                ...
                // SINKRONISASI STATUS DENGAN POIN
                $lowerDesc = array_map('strtolower', $matchingRulesDesc);
                if (in_array('telat', $lowerDesc) && !$usedToken) {
                    $status = 'telat';
                } elseif (in_array('alpha', $lowerDesc)) {
                    $status = 'alpha';
                }
    ...
    /*
    ===============================
    AMBIL DATA WAJAH GURU
    ===============================
    */
    public function faceData()
    {
        $teachers = Teacher::whereNotNull('face_data')->get(['id', 'name', 'face_data']);
        return response()->json([
            'status' => 'success',
            'data' => $teachers
        ]);
            ...
            if (!$attendance) {
                ...
                if ($pointsEarned != 0) {
                    ...
                    \App\Models\PointLedger::create([
                        ...
                        'amount'           => $pointsEarned,
                        'current_balance'  => $teacher->point_balance,
                        'description'      => 'Absen Masuk (Face ID): ' . implode(', ', $matchingRulesDesc),
                    ]);
                }
